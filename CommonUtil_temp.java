...
public class CommonUtil
{
  ...
  /**
   * ???쒑린?   * CHECKisdigit("12a","12a".length())
   *
   * @param string
   * @param piStrLen
   * @return
   ...
   * '-' ?닌됲뀋?癒? 揶쎛筌왖筌롫똻苑?筌ｃ꺂苡뀐쭪??얜챷???곸뱽 ?닌뗫릭??   *
   * @param request_name
   * @return
   */
  ...
  /**
   * @PARAM str 獄쏅떽???삳뮉 ?얜챷???곸뱽 揶쎛筌??癒?궚
   * @PARAM pattern 筌≪뼚???얜챷???   * @PARAM replace 獄쏅떽??빳??얜챷???   * str="aaaaa k k"
   * pattern="k k"
   * replace="w w"
   * return = "aaaaa w w"
   */
  static String replace(String str, String pattern, String replace) {
    ...
    int s = 0; // 筌≪뼐由???뽰삂???袁⑺뒄
    int e = 0; // StringBuffer??append ???袁⑺뒄
    StringBuffer result = new StringBuffer(); // ?醫롫뻻 ?얜챷?????욱뀑????
  ...
  public static final boolean runtime(String cmd){
    try{
      ...
      //System.out.println("??됱뇚獄쏆뮇源?+ex.getMessage());
      ex.printStackTrace();
      return false;
    }
    return true;
  ...
  /**
   * System.out.println??곗쨮 ?遺얇늺??癰귣똻肉т빳???   *
   * @param     String str
   * @return    void
   */
  ...
  /**
   * len 癰귣???疫??얜챷???곸뵬 野껋럩???癒?뀲??
   *
   * @param     str ??낆젾 ?얜챷???   * @param     len ?얜챷???疫뀀챷??   * @return    String
   */
  ...
  /**
   * ??낆젾??String?????, ??곕솁甕? ??ъ쁽?紐???筌ｋ똾寃??뺣뼄.
   *
   * @param     String data
   * @return    int
   *            0??野껋럩????ъ쁽,1??野껋럩????곕솁甕? 2??野껋럩?????
   */
  public static final int checkHan(String data) {
    int code = (int)data.charAt(0);
    int result = 1;	// ??곕솁甕곕끃??野껋럩??    if(code > 127){
      result = 2;	// ?????野껋럩??    } else if(code >= 48 && code <= 57){
      result = 0;	// ??ъ쁽??野껋럩??    }
    return result;
  }
  ...
  /**
   * 雅뚯눘堉깍쭪???쎈뱜筌띻낯??????紐? ?癒?뼊 // 雅뚯눘堉깍쭪??얜챷???곷툧?癒?퐣 ??롪돌????????????덈뼄筌?true
   * @param  String data
   * @return boolean
   *
   */
  ...
  {
    // ?얜챷???곸벥 疫뀀챷??? ?얜챷???곸벥 獄쏅뗄??紐껉컳??곸벥 疫뀀챷?좂몴???쑨???곴퐣 筌ｋ똾寃?    if(str.length() == str.getBytes().length){
  ...
  // org : 1234567  tar : 345 -> org?癒?퐣 tar ??쎈뱜筌띻낯????釉??뤿선 ??덈뮉筌왖 ?類ㅼ뵥??뺣뼄.
  ...
  public synchronized static String GetEnvValue(String envfilename,String tmp)
  {
    ...
    try {
      ...
      System.out.println("GetEnvValue() ?癒?퐣 ??띻펾???샒??2 ??됱뇚獄쏆뮇源? );
      e.printStackTrace();
    }
    finally{
      try{
  ...
  public static final void printOne(File f)
  {
    ...
    {
      System.out.print("\t\t\t\t\t ???뵬??鈺곕똻???? ??녿뮸??덈뼄.");
    }

    System.out.println("???뵬筌?: " + f.getName());
  }
  ...
  */
  ...
  /**
   * ??뽰삂??볦퍢???ル굝利??볦퍢??嚥↔퉫而??곗쨮 ??뤾텦??Interval 筌△뫁?좂몴??④쑴沅??뺣뼄.
   *  - ??우쒀?? ?λ뜄??袁⑥쨮 ??깃텦??뺣뼄.
   */
  ...
  /**
   * ??筌롫뗄???뺣뮉 癰귣?沅∽쭪???쎈뱜筌띻낯??嚥≪뮄????뵬??疫꿸퀡以??뺣뼄.
  ...
  /**
   * ????λ땾??property file (C:\MYTEST\CHB\src\webserver\webserver.properties)嚥?   * ?봔????쎄괠?믪눛?껆뙴??뱜?遺얠젂?醫듼봺,??苑?嚥≪뮄??遺얠젂?醫듼봺???類ｋ궖????뚮선?? ???館釉??
   * @return boolean - true, if the file exits and parameters are specified.
   *					false, otherwise.
   *-----------------------------------------------------------------------------
   * webserver.properties ???뵬????곸뒠
  ...
  */
  ...
  /**
   * ??쇰선??double??뤿퓠 ???????땾 2筌욌챷?꾤뵳??댐쭪? ??녿선??용┛
   *
   * @param pattern : ######.##
   * @double        : 123456.789
   */
  ...
  /**
  ??彛????ъ쁽???類ㅻ뻼??
  ** ?類ㅻ뻼?????숲 ??쎈뱜筌띻낯肉??????롫뮉 ?諭???얜챷??**
  0 : ??롪돌????ъ쁽
  # : ??롪돌????ъ쁽??援?0 ??野껋럩???⑤벉媛??곗뮆??  . : ???땾?癒?뱽 ????????얜챷??  , : ??ъ쁽??comma 嚥??브쑬???롫뮉 ?얜챷??  ; : ???쉘???臾믩땾 ???쉘?????땾 ???쉘??곗쨮 ??롫땸
  - : ???벥 ?봔?紐? ????????얜챷??  % : 100 ???④퉲釉?????깃쉽?紐껋쨮 ?????  疫꿸퀬? ?얜챷??: ???쉘????뚮츟???????????덈뼄.
  ' : ???쉘????뚮츟?癒?퐣 ?諭???얜챷?꾤몴?癰귣똾???얜챷?꾣에??????????뉗쓺 ??  */
  ...
  public static final int getRandomNum(int k)
  {
    ...
    // 17?? ??곗뺘?怨몄뵥 ??쏅땾(Prime Number)?紐껊쑓
    // Prime Number嚥???곷튊 ??뽯땾揶쎛 ?ⓥ몿?х뙴??브쑵猷??몃빍??
    // ?얠눖以???삘뀲 ??롮쨮??猷??우뮇媛??щ빍?? ??쏅땾筌????ル뿪???.

    int randomNumber = ran.nextInt(); // ??쇱젫嚥???뽯땾??롪돌??筌띾슢諭??沅???臾믩씜??낅빍??
    if( randomNumber < 0 ) randomNumber*=(-1);

    return (randomNumber % k);
    // ??욧탷 0~100 ???졿에???뽯땾??????域뱀뮇???롮젻??    // 野껉퍔?????쇱젫嚥???뽯땾?????怨???용뮉 ?룐뫂???낅빍??
  }

  /////////////////////////////////////////////////////////////////////////////
  // ??롪돌?????뵬????뚮선 ??깆뵥??μ맄癰귢쑬以??곗뮆???뺣뼄.
  /////////////////////////////////////////////////////////////////////////////
  ...
  {
    String ConfigPath =
        //"C:\\MYTEST\\CHB\\src\\?癒곕７-癰귣챷???Ver1.0\\Config.INI";
    ...
    try {
      ...
      if (f.exists()) {
        System.out.println("???뵬????由?: " +f.length());
        System.out.println("???뵬"+f.getName()+"? 鈺곕똻???몃빍??");
        ...
        System.out.println("???뵬" + fname + "? 鈺곕똻???? ??녿뮸??덈뼄");
        return -1;
      }
    }
    catch (Exception e) {
...
/*
  ...
  appendFileWrite("./hhhhhh.txt","???aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
 ...
 */
  public boolean savingINI()
  {
    try
    {
      ...
      p.store(new FileOutputStream("./Config.ini"), "INI REFERENCE[ConfigReference.ini]");
      return true;
    }
    catch(Exception e)
    {
