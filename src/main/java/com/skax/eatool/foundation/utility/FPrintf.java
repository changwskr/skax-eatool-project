...
/**
 * FPrintf allows the formatting of an array of
 ...
 * thrown and the instance of FPrintf is not
 * created.</p>
 *<p>
...
public class FPrintf {
   ...
   * malformed.
   */
  public FPrintf(Locale locale,String fmtArg)
      throws IllegalArgumentException {
  ...
  {
    System.out.println(new FPrintf("Pre %s Post").sprintf("target"));
    System.out.println(new FPrintf("Pre %s Post%%").sprintf("target") );

    System.out.println(new FPrintf("|%-10.10s|").sprintf("target") );
    ...
    System.out.println( new FPrintf("[%3.3s]").sprintf(CommonUtil.Float2Str(fitv) ));


    new FPrintf("Pre%sPost").sprintf("target");
    new FPrintf("\'%6s\'").sprintf("12345");
    new FPrintf("\'%7s\'").sprintf("12345");
    // '12345  '
    new FPrintf("\'%-7s\'").sprintf("12345");
    // '    123'
    new FPrintf("\'%7.3s\'").sprintf("12345");
    // '123    '
    new FPrintf("\'%-7.3s\'").sprintf("12345");

    // '0xffff'
    new FPrintf("\'%hx\'").sprintf(-1) ;
    // '0xffffffff'
    new FPrintf("\'%x\'").sprintf(-1L) ;
    // '0XFFFFFFFFFFFFFFFF'
    new FPrintf("\'%lX\'").sprintf(-1L);
    // 'a'
    new FPrintf("\'%c\'").sprintf(97);


/*
    Short s=(Short)Short.MAX_VALUE ;
    // '-32768'
    new FPrintf("`%d\'").sprintf(s);
    // '100000'
    new FPrintf("\'%ho\'").sprintf(s);
    // '37777700000'
    new FPrintf("\'%o\'").sprintf(s);
    // '1777777777777777700000'
    new FPrintf("\'%lo\'").sprintf(s);
    // '8000'
    new FPrintf("\'%hx\'").sprintf(s);
    // 'ffff8000'
    new FPrintf("\'%x\'").sprintf(s);
    // 'ffffffffffff8000'
    new FPrintf("\'%lx\'").sprintf(s);
*/
    // '     +1'
    new FPrintf("\'%+7d\'").sprintf(1);
    // '     -1'
    new FPrintf("\'%+7d\'").sprintf(-1);
